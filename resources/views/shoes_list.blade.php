    ...
    <section class="container mx-auto mt-28 mb-6 px-4 md:px-12">
        @if (count($shoes) > 0)
            <div class="flex justify-between">
                @if (!isset($brand))
                    <h1 class="text-3xl font-medium mb-7">All Shoes</h1>
                @else
                    <h1 class="text-3xl font-medium mb-7">{{ucfirst($brand)}} Shoes</h1>
                @endif
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                @foreach ($shoes as $item)    
                <div class="w-full border rounded-lg overflow-hidden shadow-md">
                    <a href="{{ url('details/' . $item->shoes_id) }}">
                        <div class="w-full h-72 flex items-center justify-center bg-gray-50 object-cover object-center group-hover overflow-hidden">
                            <img class="hover:opacity-[0.95] h-72 hover:scale-105 transition ease-in-out duration-200"
                                src="{{ $item->shoes_image }}" alt="Shoes">
                        </div>
                        <div class="mt-4 px-7 pt-4 pb-7">
                            <h2 class="text-gray-900 font-semibold text-lg">{{ $item->shoes_name }}</h2>
                            <h2 class="text-red-900 font-semibold text-lg my-3"><span class="text-sm font-thin">Rp </span>{{ str_replace(',', '.', number_format($item->shoes_price)) }}</h2>
            ...
            @else
            <div class="mb-80 flex flex-col justify-center items-center">
                <h1 class="text-gray-900 text-center text-3xl font-bold mb-1">Sorry, we couldn't find any shoes for the brand "<strong>{{ $brand }}</strong>" :( </h1>
                <p class="text-gray-700 mb-7">Please try searching for a different brand.</p>
                <a href="{{ url('/') }}">
                    <button type="button"
                                    class="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5">Back to home</button>
                </a>
            </div>
